...
export default function AudienceView({ room, identifier }: AudienceViewProps) {
  ...
  // Pick random available contribution using database function
  const pickNewContribution = async () => {
    setIsLoading(true)

    const { data, error } = await supabase.rpc('get_random_available_contribution', {
      p_room_id: room.id,
      p_identifier: identifier
    })

    if (error) {
      console.error('Error fetching available contribution:', error)
      setIsLoading(false)
      return
    }
    ...
    const combination = data[0]

    // Find the image and question from config
    const image = config.images.find(img => img.image_id === combination.image_id)
    const question = config.questions.questions.find(q => q.id === combination.question_id)
  ...
  if (isLoading || !currentImage || !currentQuestion) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <p className="text-gray-400">
          {!isLoading && !currentImage && !currentQuestion
            ? 'All available contributions completed! Thank you.'
            : 'Loading...'}
        </p>
      </div>
    )
  ...
  const rangeStyle: CSSProperties = {
    ['--thumb-size' as any]: `${selectorSize}px`,
    ['--thumb-color' as any]: hasInteracted ? selectorColor : '#6b7280',
  }
